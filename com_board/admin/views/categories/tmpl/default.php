...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_board&task=categories.saveOrderAjax&tmpl=component';
...
<form action="<?php echo Route::_('index.php?option=com_board&view=categories'); ?>" method="post" name="adminForm"
	  id="adminForm">
		...
		<?php else : ?>
			<table id="categoriesList" class="table table-striped">
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canEdit = $user->authorise('core.edit', '#__board_categories.' . $item->id);
					$canChange = $user->authorise('core.edit.state', '#__board_categories' . $item->id);
					$item->parents = '';
						...
						<td class="center">
							<div class="btn-group">
								<a class="btn btn-micro hasTooltip" title="<?php echo Text::_('JACTION_EDIT'); ?>"
								   href="<?php echo Route::_('index.php?option=com_board&task=category.edit&id=' . $item->id); ?>">
