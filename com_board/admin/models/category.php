...
class BoardModelCategory extends AdminModel
{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			$item->tags->getTagIds($item->id, 'com_board.category');
	 ...
	 * @since  1.0.0
	 */
	public function getTable($type = 'Categories', $prefix = 'BoardTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		$form = $this->loadForm('com_board.category', 'category', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_board.category.' . (int) $id))
			|| ($id == 0 && !$user->authorise('core.edit.state', 'com_board')))
		{
		...
		$form->setFieldAttribute('icon', 'saveurl', 'index.php?option=com_board&task=category.updateImages&field=icon&id=' . $id);
	...
	{
		$app  = Factory::getApplication();
		$data = Factory::getApplication()->getUserState('com_board.edit.category.data', array());
		if (empty($data))
		{
			$data = $this->getItem();
		...
		$this->preprocessData('com_board.category', $data);
	...
	public function save($data)
	{
		...
		{
			$msg   = Text::_('COM_BOARD_CATEGORY_SAVE_WARNING');
