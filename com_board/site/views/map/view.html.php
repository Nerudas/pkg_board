...
class BoardViewMap extends HtmlView
{
	/**
	 * Category object
	 *
	...
	public function display($tpl = null)
	{
		...
		if ($active)
		{
			...
			if (strpos($currentLink, 'view=category') && strpos($currentLink, '&catid=' . (string) $category->id))
			{
				...
				elseif ($layout = $category->params->get('map_layout'))
				{
			...
			else
			{
				...
				if ($layout = $category->params->get('map_layout'))
				{
					$this->setLayout($layout);
				}
			}
		...
		// Set Map Parametrs
		$this->mapParams['layout'] = $layout;
		$this->document->addScriptOptions('boardMap', $this->mapParams);
	...
	protected function _prepareDocument()
	{
		...
		if ($menu && ($menu->query['option'] !== 'com_board' || $menu->query['view'] !== 'map' || $id != $category->id))
		{
			if ($category->title)
			{
			...
			while ($parent && $parent->id > 1 &&
				($menu->query['option'] !== 'com_board' || $menu->query['view'] === 'map' || $id != $parent->id))
			{
				$path[] = array('title' => $parent->title, 'link' => $parent->mapLink);
