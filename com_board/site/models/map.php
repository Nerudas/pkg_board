...
class BoardModelMap extends ListModel
{
	/**
	 * Type data
	 *
	...
	public function getMapParams()
	{
		if (!is_array($this->_mapParams))
		{
			...
			if (!empty($cluster))
			{
				...
				if (!empty($cluster->gridSize))
				{
					$params['cluster']['gridSize'] = $cluster->gridSize;
				}
				if (!empty($cluster->minClusterSize))
				{
					$params['cluster']['minClusterSize'] = $cluster->minClusterSize;
				}
				if (!empty($cluster->numberColor))
				{
					$params['cluster']['numberColor'] = $cluster->numberColor;
				}
				if (!empty($cluster->clickOpacity))
				{
					$params['cluster']['clickOpacity'] = $cluster->clickOpacity;
				}

				if (!empty($cluster->icon) && (!empty($cluster->icon->href) || !empty($cluster->icon->size) || !empty($cluster->icon->offset)))
				{
					$icon = new stdClass();
					if (!empty($cluster->icon->href))
					{
						$icon->href = Uri::base('true') . '/' . $cluster->icon->href;
					}
					if (!empty($cluster->icon->size))
					{
						$icon->size = json_decode($cluster->icon->size);
					}
					if (!empty($cluster->icon->offset))
					{
						$icon->offset = json_decode($cluster->icon->offset);
					}

					$params['cluster']['clusterIcons'] = array($icon);
				}
			}
	...
	public function getFilterForm($data = array(), $loadData = true)
	{
		...
		$form->setFieldAttribute('category', 'view', 'map', 'filter');
		$form->removeField('allregions', 'filter');
