...
class JFormFieldBoardCategory extends JFormFieldList
{
	...
	public function setup(SimpleXMLElement $element, $value, $group = null)
	{
		...
		{
			$this->value = ($view == 'list' || $view == 'map') ?
				$app->input->get('id', 1) : $app->input->get('catid', 1);
		}
	...
	protected function getOptions()
	{
		...
		foreach ($categories as $i => $category)
		{
			...
			$option->value = (!$this->links) ? $category->id :
				Route::_('index.php?option=com_board&view=' . $this->view . '&id=' . $category->id);
